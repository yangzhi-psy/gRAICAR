...
function obj = findComp_keepFSM (obj)
...
for sub = 1:obj.setup.subNum   
	...
	% converted the negative values in the FSM so that RCs can be picked up again
end

% extract the corresponding row and column
row = abs (obj.result.MICM(mpos_x, :))'+abs (obj.result.MICM(:,mpos_x))-obj.result.thr;
col = abs (obj.result.MICM(:, mpos_y))+abs (obj.result.MICM(mpos_y, :))'-obj.result.thr;
