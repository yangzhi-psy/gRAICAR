function [A, W, flagConverge] = fpica(X, whiteningMatrix, dewhiteningMatrix, approach, ...
...
if approachMode == 1,
  ...
  for round = 1:maxNumIterations + 1,
    ...
    %if b_verbose
      if round == 1
        fprintf(' Step no. %d \r', round);
      else
        fprintf(' Step no. %d, change in value of estimate: %.3g \r', round, 1 - minAbsCos);
      end
    %end
